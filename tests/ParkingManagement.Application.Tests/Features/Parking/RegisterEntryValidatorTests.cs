...
{
    private readonly RegisterEntryValidator _validator = new();
    ...
    {
        // Arrange
        var request = new RegisterEntryRequest(licensePlate, DateTime.UtcNow);
    ...
    [Fact]
    public async Task Validar_DeveFalhar_QuandoEntryTimeNaoInformado()
    {
        // Arrange
        var request = new RegisterEntryRequest("ZUL0001", default);
        ...
        // Assert
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(e => e.ErrorMessage == ApplicationErrorMessages.Parking.EntryTimeObrigatorio);
    }
}
